 ...
 */
defined('THEMOSIS_LOGGER_TD') ? THEMOSIS_LOGGER_TD : define('THEMOSIS_LOGGER_TD', 'themosis-logger');
 ...
 */
$vars = [
    'slug' => 'themosis-logger',
    'name' => 'Themosis Logger',
    'namespace' => 'com.keltiecochrane.logger',
    'config' => 'com_keltiecochrane_logger',
];
...
add_action('admin_notices', function () use ($vars) {
    if (!class_exists('\Themosis\Foundation\Application')) {
        printf('<div class="notice notice-error"><p>%s</p></div>', __('This plugin requires the Themosis framework in order to work.', THEMOSIS_LOGGER_TD));
    }
    ...
    if (!current_theme_supports($vars['slug']) && current_user_can('switch_themes')) {
        printf('<div class="notice notice-warning"><p>%s<strong>%s</strong></p></div>', __('Your application do not handle the following plugin: ', THEMOSIS_LOGGER_TD), $vars['name']);
    }
});
...
container('action')->add('plugins_loaded', function () use ($vars) {
	 ...
	 */
	load_plugin_textdomain(THEMOSIS_LOGGER_TD, false, trailingslashit(dirname(plugin_basename(__FILE__))).'languages');
